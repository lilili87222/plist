...
func Unmarshal(data []byte, v interface{}) (format int, err error) {
	// Convert XML 1.1 to XML 1.0 for compatibility with Go's xml parser
	// Go's encoding/xml doesn't support XML 1.1
	if bytes.HasPrefix(data, []byte("<?xml version=\"1.1\"")) {
